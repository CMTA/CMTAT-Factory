...
import {CMTATLightBeaconFactoryBase} from "../modules/deployment/CMTATLightBeaconFactoryBase.sol";
...
/**
* @notice Factory to deploy CMTAT behind a beacon proxy (CMTAT Light), gated by a single owner.
* @dev Single-owner variant of `CMTAT_LIGHT_BEACON_FACTORY`, sharing its deployment logic and differing only in
...
* separated duties, use `CMTAT_LIGHT_BEACON_FACTORY`. The choice is made at deployment and cannot be changed at a
* deployed address.
*/
contract CMTAT_LIGHT_BEACON_FACTORY_Ownable2Step is CMTATLightBeaconFactoryBase, CMTATFactoryOwnable2Step {
    /**
    ...
    */
    constructor(
        ...
        address beaconOwner,
        bool useCustomSalt_
    ) CMTATLightBeaconFactoryBase(implementation_, beaconOwner, useCustomSalt_) Ownable(initialOwner) {}
