 ...
 */
abstract contract CMTATFactoryRoot is
    ContractVersion,
    ...
    ICMTATFactory
{
     ...
     */
    bool public immutable override useCustomSalt;
    /**
    ...
    function CMTATProxyAddress(
        ...
        return
            cmtatCounterId_ < cmtatsList.length
                ? cmtatsList[cmtatCounterId_]
                : address(0);
    }
    ...
    function supportsInterface(
        bytes4 interfaceId
    ) public view virtual override(ContractVersion) returns (bool) {
        return
     ...
     */
    function nextDeploymentSalt()
        public
        view
        virtual
        override
        returns (bytes32 saltBytes)
    {
     ...
     */
    function isCustomSaltUsed(
        bytes32 salt
    ) public view virtual override returns (bool used) {
     ...
     */
    function _checkAndDetermineDeploymentSalt(
        bytes32 deploymentSalt
    ) internal virtual returns (bytes32 saltBytes) {
        if (useCustomSalt) {
    ...
    function _deployAndRegisterProxy(
        ...
        bytes32 deploymentSalt
    ) internal virtual returns (address cmtatAddress) {
     ...
     */
    function _computeDeploymentSalt(
        bytes32 deploymentSaltInput
    ) internal view virtual returns (bytes32 saltBytes) {
