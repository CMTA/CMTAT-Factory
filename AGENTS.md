...
- `CMTAT_LIGHT_TP_FACTORY` for CMTAT Light Transparent proxies
- `CMTAT_LIGHT_BEACON_FACTORY` for CMTAT Light Beacon proxies
...
CMTAT_LIGHT_TP_FACTORY      - deploys TransparentUpgradeableProxy instances pointing at CMTATUpgradeableLight
CMTAT_LIGHT_BEACON_FACTORY  - deploys BeaconProxy instances backed by one UpgradeableBeacon pointing at CMTATUpgradeableLight
```
...
- `CMTAT_LIGHT_TP_FACTORY`
  Uses `TransparentUpgradeableProxy`, encodes `CMTATUpgradeableLight.initialize(...)`, and requires `proxyAdminOwner` per deployment.

- `CMTAT_LIGHT_BEACON_FACTORY`
  Creates one `UpgradeableBeacon` in the constructor and deploys `BeaconProxy` instances.
  If `implementation_` is zero, it deploys a fresh `CMTATUpgradeableLight` implementation internally before creating the beacon.
...
5. Factory emits `CMTATDeployed(proxy,deployer,id,salt)`, increments `cmtatCounterId`, and appends the proxy to `cmtatsList` (the array doubles as the `id => address` registry, indexed by `cmtatCounterId`).
...
For CMTAT Light factories, the initializer uses the smaller `CMTAT_LIGHT_ARGUMENT`:
...
- `test/Light/` covers CMTAT Light Transparent and Beacon factory behavior.
